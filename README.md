...
- 新增 Paper Trading：支持纸面账户、手动下单、历史结论回放、佣金/滑点模拟和实时权益/价格曲线。
...
模拟盘（纸面账户）页面展示实时行情刷新、权益/价格曲线和手动下单区：

![纸面账户实时曲线](assets/readme/paper-account-realtime.jpg)
...
- 纸面交易：查看纸面账户、提交手动订单、从历史任务结论回填交易参数，并追踪成交、持仓和实时权益曲线。
